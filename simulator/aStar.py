...
def astar(maze, start, goal):
    """! This runs the astar algorithm and returns a path
    to the given goal in the given maze
    @param start start is starting position
    @param goal goal is the ending position
    @return a list of tuples as a path from the given start
    or None for no path
    """
    if maze[start[0]][start[1]] == 1 or maze[goal[0]][goal[1]] == 1:
        return None #no path if starting position is not walkable
